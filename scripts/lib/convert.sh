...
_download_raw_config() {
    ...
    local url=$2
    local proxy_args=()
    [ -n "${SUB_FETCH_PROXY:-}" ] && proxy_args=(--proxy "$SUB_FETCH_PROXY")
    ...
    # 指定代理时不允许 wget 回退绕过代理；curl 是安装器的必需依赖。
    if [ "$rc" -ne 0 ] && [ -z "${SUB_FETCH_PROXY:-}" ]; then
        wget \
            --no-verbose \
...
_download_convert_config() {
    ...
    (
        # subconverter 自己负责获取原订阅。显式代理通过环境传给它；
        # 本地控制请求始终绕过代理，避免把 127.0.0.1 发往上游。
        if [ -n "${SUB_FETCH_PROXY:-}" ]; then
            export http_proxy="$SUB_FETCH_PROXY" HTTP_PROXY="$SUB_FETCH_PROXY"
            export https_proxy="$SUB_FETCH_PROXY" HTTPS_PROXY="$SUB_FETCH_PROXY"
            export all_proxy="$SUB_FETCH_PROXY" ALL_PROXY="$SUB_FETCH_PROXY"
        fi
