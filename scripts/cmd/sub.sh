...
_sub_validate_proxy() {
    ...
    [ -z "$proxy" ] && return 0
    case "$proxy" in
    http://* | https://* | socks4://* | socks4a://* | socks5://* | socks5h://*) return 0 ;;
    esac
    _errorcat "代理地址必须包含协议，例如 http://127.0.0.1:7890 或 socks5h://127.0.0.1:1080"
    return 1
}
...
sub_add() {
    local use_after_add=false name='' url='' strategy=auto SUB_FETCH_PROXY=''
...
Options:
  ...
  -u, --use           添加后立即使用该订阅
  --proxy <URL>       通过指定 HTTP/SOCKS 代理下载，并保存供后续更新使用
...
EOF
    ...
    _sub_validate_proxy "$SUB_FETCH_PROXY" || return 1
...
${_SUB_DL_DEBUG_HINT:-转换日志：$BIN_SUBCONVERTER_LOG}"
    ...
    _with_profiles_lock _sub_add_locked "$name" "$url" "$use_after_add" "$_SUB_DL_FILE" "$FETCH_USERINFO" "$FETCH_FILENAME" "$SUB_FETCH_PROXY"
}

# 临界区：确定唯一名称/路径、落盘、写入元数据（可选立即启用）
_sub_add_locked() {
    local name=$1 url=$2 use_after_add=$3 dl_file=$4 userinfo=$5 filename=$6 fetch_proxy=$7
    ...
    PROFILE_NAME=$name PROFILE_PATH=$profile_path PROFILE_URL=$url PROFILE_UPDATED=$now \
        PROFILE_USERINFO=$userinfo PROFILE_PROXY=$fetch_proxy \
        "$BIN_YQ" -i '
            .profiles = (.profiles // []) +
            [{
              ...
              "proxy": strenv(PROFILE_PROXY)
            }]
        ' "$CLASH_PROFILES_META"

    if [ "$use_after_add" = true ] && ! _sub_use_locked "$name"; then
        # 配置校验失败时 _sub_use_locked 已恢复 BASE/runtime，新增记录也应撤销，
        # 让 add --use 保持事务语义。若 use 已写入则说明只是服务重启失败，
        # 此时配置已经切换成功，不能删除正在使用的订阅。
        if [ "$(_sub_current)" != "$name" ]; then
            /usr/bin/rm -f "$profile_path"
            PROFILE_NAME=$name "$BIN_YQ" -i \
                'del(.profiles[] | select(.name == strenv(PROFILE_NAME)))' \
                "$CLASH_PROFILES_META"
            _logging_sub "↩️ 添加并启用失败，已撤销订阅：[$name] $url"
            _errorcat "订阅 [$name] 启用失败，新增记录已撤销"
        fi
        return 1
    fi
...
Usage:
  clashctl sub update [name] [--all] [--proxy <URL> | --no-proxy]
                      [--convert | --raw] [-t <秒>] [--ua <UA>]
...
Options:
  --all        更新全部订阅
  --proxy URL  本次通过该代理下载，并保存供后续更新使用
  --no-proxy   本次直接下载，并清除已保存的代理
  --convert    始终经 subconverter 转换（默认 auto：原生有效则直用，否则回退转换）
  --raw        仅下载，不转换（校验失败即失败）
  -t, --timeout <秒>  单次命令级下载超时（默认 ${CLASHCTL_SUB_TIMEOUT:-20} 秒，可在 .env 全局配置）
  --ua <UA>    单次命令级下载 UA（默认 ${CLASHCTL_SUB_UA:-clash-verge/v2.4.0}，可在 .env 全局配置）
...
EOF
    ...
    local strategy=auto name='' all=false proxy_override='' proxy_set=false
    while [ $# -gt 0 ]; do
        case "$1" in
        --all)
            all=true
    ...
    _sub_validate_proxy "$proxy_override" || return 1
    ...
    [ "$all" = true ] && {
        ...
        for n in "${names[@]}"; do
            _sub_update_one "$n" "$strategy" "$proxy_override" "$proxy_set" || rc=1
        done
        return "$rc"
    }
    ...
    _sub_update_one "$name" "$strategy" "$proxy_override" "$proxy_set"
}

# 更新单个订阅：下载/校验在锁外，提交在锁内（供单个与 --all 复用）
_sub_update_one() {
    local name=$1 strategy=$2 proxy_override=${3:-} proxy_set=${4:-false} SUB_FETCH_PROXY
    _sub_has "$name" || {
        _errorcat "订阅不存在：$name"
        return 1
    }
    ...
    if [ "$proxy_set" = true ]; then
        SUB_FETCH_PROXY=$proxy_override
    else
        SUB_FETCH_PROXY=$(_sub_get "$name" proxy)
    fi
...
${_SUB_DL_DEBUG_HINT:-转换日志：$BIN_SUBCONVERTER_LOG}"
    ...
    _with_profiles_lock _sub_update_locked "$name" "$url" "$path" "$_SUB_DL_FILE" "$FETCH_USERINFO" "$SUB_FETCH_PROXY" "$proxy_set"
}

_sub_update_locked() {
    local name=$1 url=$2 path=$3 dl_file=$4 userinfo=$5 fetch_proxy=$6 proxy_set=$7
    ...
    [ -n "$userinfo" ] && PROFILE_NAME=$name PROFILE_USERINFO=$userinfo "$BIN_YQ" -i \
        ...
        "$CLASH_PROFILES_META"
    [ "$proxy_set" = true ] && PROFILE_NAME=$name PROFILE_PROXY=$fetch_proxy "$BIN_YQ" -i \
        '(.profiles[] | select(.name == strenv(PROFILE_NAME)) | .proxy) = strenv(PROFILE_PROXY)' \
        "$CLASH_PROFILES_META"
...
Commands:
  add [-n NAME] <url>   添加订阅（-u 立即使用，--proxy URL 经代理下载）
