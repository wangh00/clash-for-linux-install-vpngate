...
[ -f "$CLASHCTL_SRC/.env" ] || {
    [ -f "$CLASHCTL_SRC/.env.example" ] &&
        /bin/cp "$CLASHCTL_SRC/.env.example" "$CLASHCTL_SRC/.env"
}
. "$CLASHCTL_SRC/.env"
. "$CLASHCTL_SRC/.env.install"
# 可选的本机私有覆盖文件。它被 .gitignore 排除，适合保存订阅 URL、代理密码
# 等不应提交到仓库的安装参数。
[ ! -f "$CLASHCTL_SRC/.env.install.local" ] || . "$CLASHCTL_SRC/.env.install.local"
...
valid_required() {
    local required_cmds=("xz" "pgrep" "pkill" "curl" "tar" 'unzip' 'gzip' 'shuf' 'awk' 'base64' 'sort')
    local missing=()
...
_github_url_candidates() {
    local url=$1 proxy proxy_list="${GH_PROXY_LIST:-}" seen=' '

    # 兼容旧版配置：若用户仍设置了 GH_PROXY，则优先使用它。
    [ -n "${GH_PROXY:-}" ] && proxy_list="${GH_PROXY} ${proxy_list}"
    ...
    [ "${GH_DIRECT_FALLBACK:-0}" = 1 ] && printf '%s\n' "$url"
    return 0
}

_fetch_latest_tag() {
    local repo=$1 api_url body tag candidate
    ...
    done < <(_github_url_candidates "$api_url")
    return 1
}

_download_github_asset() {
    local url=$1 target=$2 candidate tmp="${target}.part"

    while IFS= read -r candidate; do
        [ -n "$candidate" ] || continue
        /bin/rm -f "$tmp"
        _okcat '⏳' "正在尝试下载：$candidate"
        if curl \
            --progress-bar \
            --show-error \
            --fail \
            ...
            _errorcat "下载内容不是有效压缩包，切换下一个镜像：$candidate" || true
        else
            _errorcat "下载失败，切换下一个镜像：$candidate" || true
        fi
    done < <(_github_url_candidates "$url")

    /bin/rm -f "$tmp"
    _errorcat "所有 GitHub 下载源均失败：$url"
    return 1
}
...
download_zip() {
    ...
    *)
    ...
    for item in "$@"; do
        ...
        _okcat '📦' "正在下载：${item}"
        _download_github_asset "$url" "$target" || exit
        target_zips+=("$target")
    done
